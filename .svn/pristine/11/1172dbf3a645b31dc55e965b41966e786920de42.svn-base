...
#define DEFAULT_IPADDR          0x0A0101B1
#endif
...
#define DEFAULT_NET_MASK        ((255u << 24) | (255 << 16) | (255 << 8) | 0)
#endif
...
UIEthernetScanReceive(void)
{
    ...
    while(g_ulUIEthernetReceiveRead != g_ulUIEthernetReceiveWrite)
    {
        ...
        {
        	//
        	// send back a invalid command response
        	//
            g_pucUIEthernetResponse[0] = TAG_ERROR;
            g_pucUIEthernetResponse[1] = 0x04;
            g_pucUIEthernetResponse[2] = 0xff;
                                    ...
                                    UIETHERNET_MAX_RECV])
        {
            ...
            {
                g_pucUIEthernetResponse[0] = TAG_STATUS;
                g_pucUIEthernetResponse[1] = 4;
                g_pucUIEthernetResponse[2] = CMD_UPGRADE;
                //
                // Send the response.
                //
                UIEthernetTransmit(g_pucUIEthernetResponse);
            ...
            case CMD_SET_PARAM_VALUE:
            {
                ...
                // Fill in the response.
                //
                g_pucUIEthernetResponse[0] = TAG_RDONLY;
                g_pucUIEthernetResponse[1] = 0x04;
...
UIEthernetSendRealTimeData(void)
{
    ...
    for(ulItem = 0, ulPos = 2; ulItem < g_ulUINumRealTimeData; ulItem++)
    {
        ...
        if(g_pulUIRealTimeData[ulIdx / 32] & (1 << (ulIdx % 32)))
        {
            ...
            //
        	if(g_sUIRealTimeData[ulItem].ucSize > 4)
        	{
        	    memcpy(pucValue, g_sUIRealTimeData[ulItem].pucValue, g_sUIRealTimeData[ulItem].ucSize);
        	}
        	else
        	{
...
{
	int i;
	unsigned char rtDataItem[] = {DATA_MOTOR_CURRENT, DATA_BUS_VOLTAGE, DATA_ROTOR_SPEED,
			DATA_DIRECTION, DATA_FAULT_STATUS, DATA_TEMPERATURE, DATA_ANALOG_INPUT,
			DATA_TRIGGER_INFO, DATA_DIR_HALL_INFO, DATA_ROTOR_SPEED_CMD };

	for( i = 0; i < sizeof(rtDataItem); i++)
	{
		g_pulUIRealTimeData[rtDataItem[i] / 32] |= 1 << (rtDataItem[i] % 32);
	}
}
...
UIEthernetInit(tBoolean bUseDHCP)
{
    ...
    // Initialize lwIP
    //
    lwIPInit(pucMACArray, DEFAULT_IPADDR, DEFAULT_NET_MASK,DEFAULT_GATEWAY_ADDR,IPADDR_USE_STATIC);
    ...
    //
    // Initialize the real time stream data
    //
    UIInitRTData();
